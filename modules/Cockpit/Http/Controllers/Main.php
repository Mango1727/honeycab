...
class Main extends Controller
{
    ...
    public function getNearestDrivers($lat,$lng,$distanceAndTime){
        ...
        $driversQuery = User::role('driver')->where(['working' => 1]);
        ...
        try {
            ...
            foreach ($drivers as $key => $driver) {
                ...
                //Set 
                $ridecost=(($distance/1000)*$driver->restorant->getConfig('cost_per_kilometer',2)+$driver->restorant->getConfig('cost_for_start',5));
                $driver->ride_cost=$ridecost;
                $currency=$driver->restorant->currency;
                if($currency==""){
                    $currency="usd";
                }
                $driver->ride_cost_formated=Money($ridecost,$currency,$driver->restorant->do_covertion.""=="1")->format();
